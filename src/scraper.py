...
real_scraper_poc.py
--------------------
PROOF OF CONCEPT: a single route, single source, GENUINELY DOM-parsing scraper.

This intentionally does NOT do what src/scraper.py in the original codebase did
(compute a fare with a formula and call it a "quote"). Every fare in the output
comes from text actually read off the rendered page. If the page can't be read,
the run records a FAILURE for that attempt — it does not invent a number.

WHY THIS FILE EXISTS
---------------------
The previous scraper.py opened a browser, navigated to a homepage, and then
generated fares from `base_est * lead_multiplier`, completely independent of
anything on the page. That's not resilient scraping with a fallback -- it's a
fake data generator with a browser attached for show. This file replaces that
pattern with:

  1. A robots.txt check before every crawl (the problem statement explicitly
     requires this -- "remaining compliant with robots.txt").
  2. Real navigation to a search RESULTS page (not just the homepage).
  3. Real DOM extraction with multiple candidate CSS selectors, because
     aggregator sites change markup often -- when ALL candidates fail, we
     log a structured failure record and move on, we do not synthesize a price.
  4. A debug artifact (raw HTML + screenshot) saved per run, so you can open
     it and fix a selector in minutes instead of guessing blind.
  5. Polite, throttled requests with a single retry, a realistic timeout,
     and no attempt at IP rotation, CAPTCHA-solving, or session spoofing --
     those are out of scope for an ethical scraper. If the target blocks the
     scraper, honest failure is the correct behavior for a government-facing
     pricing tool, not a workaround.

IMPORTANT / HONEST CAVEAT
--------------------------
This was written and reviewed in an offline sandbox with NO network access,
so the CSS selectors below are best-effort based on how aggregator flight
search-results pages are typically structured (a repeated "flight card"
container with a distinct price element inside). They have NOT been verified
against ixigo's live DOM today. That is expected and normal for scraping --
selectors need a first live run to calibrate. Run this locally (`playwright
install chromium` then `python real_scraper_poc.py`), then:

  - If it finds 0 flight cards: open debug_output/*.html in a browser (or view
    debug_output/*.png) and update FLIGHT_CARD_SELECTORS / PRICE_SELECTORS
    near the top of the file to match what you actually see.
  - If it gets blocked (captcha/redirect/empty shell page even after wait):
    that's a legitimate finding for your documentation -- note it, and either
    add wait conditions, or pick a friendlier source. Do not "fix" this by
    reintroducing fabricated numbers.

USAGE
-----
    pip install playwright pandas
    playwright install chromium
    python real_scraper_poc.py --origin DEL --dest BOM --date 2026-10-15
"""
