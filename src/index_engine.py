...
# Map month names to standard month numbers
month_map = {
    'January': 1,
    'February': 2,
    'March': 3,
...
# Initialize automated Indian holiday calendar for any required years
india_holidays = holidays.India(years=[2025, 2026, 2027])


# Dynamic Festival Checker linked to external configuration rules
def get_config_driven_festive_multiplier(current_date):
  ...
  # Check around the current date window to capture pre/post festive spikes
  ...
  # Fallback for any general holiday recognized in official calendar not explicitly mapped
...
# 3. Simulate High-Frequency Daily Fare Quotes with Dynamic Surges
np.random.seed(42)
...
for current_date in date_range:
  ...
  # Automated Warning / High-Spike Flagging Log for Dashboard
  if festive_mult > 1.5:
    print(
...
# 4. Compute Composite APIx & Save Individual Sector-Specific Columns
...
df_apix_daily['apix_daily'] = composite_series.values * scale_factor
...
plt.plot(
    ...
    linewidth=2,
    color='#ff7f0e',
    label='Calibrated APIx (Monthly Mean)',
)
plt.title(
